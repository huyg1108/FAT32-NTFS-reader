...
class MFT_entry:
	...
	def get_active_records(self) -> 'list[MFT_entry]':
		record_list: list[MFT_entry] = []
...
class DirTree:
	def __init__(self, nodes: 'list[MFT_entry]') -> None:
		self.root = None
		self.nodes_dict: dict[int, MFT_entry] = {}
		for node in nodes:
	...
	def get_parent_record(self, record: MFT_entry):
...
class NTFS:
	...
	def visit_dir(self, path) -> MFT_entry:
		if path == "":
	...
	def get_folder_file_information(self, path):
		try:
			cur_dir = self.dir_tree.current_dir
			cur_dir = self.dir_tree.get_parent_record(cur_dir)
			record = cur_dir.find_record(path)
			obj = {}
			obj["Flags"] = record.standard_info['flags'].value
	...
	def get_file_content(self, path: str):
		...
		if record.data['resident']:
			...
			size = record.data['cluster_size'] * self.sectors_per_cluster * self.bytes_per_sector
			self.disk_file.seek(offset)
			data = self.disk_file.read(min(size, real_size))
			return data
